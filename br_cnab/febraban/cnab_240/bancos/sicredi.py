...
BOLETO_ESPECIE = {"01": "03", "02": "12", "03": "16", "05": "17"}
...
class Sicredi240(Cnab240):
    ...
    def _prepare_segmento(self, line):
        ...
        if (
            not line.src_bank_account_id.codigo_convenio
            or not line.src_bank_account_id.bra_number
        ):
            raise UserError(
        ...
        especie = BOLETO_ESPECIE.get(line.journal_id.boleto_especie, "99")

        vals["especie_titulo"] = especie
        vals["codigo_multa"] = "1"  # 1 - Valor por dia
        vlr_doc = line.debit
        juros_dia = vlr_doc * (
            self.order.journal_id.late_payment_interest / 100 / 30
        )
        ...
        vals[
            "prazo_baixa"
        ] = "060"  # Usar sempre "060" - Para baixa/devolução
        return vals
